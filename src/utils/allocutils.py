...
def cost(allocations: torch.Tensor, core_con: torch.Tensor) -> int:
  ...
  Args:
    - allocations: matrix of shape [B, Q, T] where B = batch size, Q = number of logical qubits, T = number of time slices.
  ...
  '''
  cost = core_con[allocations[:, :, :-1], allocations[:, :, 1:]].sum(dim=(1, 2)).float() # Sum over qubits and time slices # sum over Q, get [B]
  return cost
