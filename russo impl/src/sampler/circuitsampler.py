...
class CircuitSampler(ABC):
  ...
  def sample(self):
    ...
    circuit_slice_matrices is a tuple that contains a matrix for each time slice in the circuit.
    This matrix contains a 1 at position (i,j) if there is a gate between qubits i and j in that
    time slice, zero otherwise.
